...
function cm_display_order_form( $btn_text, $deceased = null ) {
	...
	if($_SERVER['REQUEST_METHOD'] === 'POST' && isset( $_POST['cm_order_nonce'] )) {
	    if(! wp_verify_nonce( $_POST['cm_order_nonce'], 'cm_place_order' )) {
	        ...
	        $errors = place_order($order, $deceased, $_POST);
        }
    }
...
function cm_products_shortcode( $atts ) {
	 ...
	 * @since 1.5.0
	 */
	$products_query_arguments = apply_filters( 'products_shortcode_query', array(
    ...
    <div id="cm-products" class="cm-products">
        <h2 class="cm-products--title"><?=esc_html($atts['title']);?></h2>
        <div class="cm-products--list">
			<?php while ( $products_query->have_posts() ): $products_query->the_post(); $product = Product::from_id(get_the_id());?>
                <div class="cm-product-wrapper">
                    <article class="cm-product">
                        ...
                        <footer class="cm-product--footer">
                            <a href="?cm_order_product=<?php the_ID();?>#cm-order-form" class="cm-product--order-link" data-product="<?php the_ID();?>">
