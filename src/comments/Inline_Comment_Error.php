...
namespace appsaloon\cm\comments;
...
class Inline_Comment_Error {
	...
	// private properties
	public $text_domain = 'wp_inline_comment_errors'; // error code for wp_error to identify errors coming from comment form
	public $comment_err; // name of session variable to store comment form $_POST data
	public $comment_post_id; // name of session variable to store wp_error

	// public properties, might be useful to other plug-in developers
	public $pre_comment_on_post_priority = 1; // for localization
	...
	); // assigned to a WP_Error in _construct
	public $default_error_msgs = array(); // holds the id of the post for the comment, not set until comment form submitted
	public $custom_error_msgs = array(); // priority of filter
	...
	*/
	public $author_validation_filter = 'wpice_validate_commentform_author';
	/*
	...
	*/
	public $url_validation_filter = 'wpice_validate_commentform_url';

	// names of custom filters
	// DOCUMENT EACH FILTER
	public $comment_validation_filter = 'wpice_validate_commentform_comment'; // wpdb/$_POST field is 'author', WordPress label is 'name'
	public $metafield_validation_filter = 'wpice_validate_commentform_metafield';
	public $get_redirect_url = 'wpice_get_redirect_url'; // wpdb/$_POST field is 'url', WordPress label is 'website'
	protected $check_post_type = true;
	private $error_code = 'comment_err';
	private $session_form_data = 'wpice_comment_form_data'; // passes url to and returns url from end user function to modify url before redirect
	private $session_wp_error = 'wpice_wp_error';
	...
	public function __construct() {
		...
		add_filter( 'pre_comment_on_post', array(
			$this,
			'validate_comment_formfields'
		), $this->pre_comment_on_post_priority );
		...
		if ( $this->check_post_type ) {
			...
			add_filter( 'wpice_get_comment_form_errors_as_list', array(
				$this,
				'get_comment_form_errors_as_list'
			) ); // returns list
		}
	 ...
	 * @since 1.0
	 */
	public function get_comment_err_die_handler() {
		return array( $this, 'comment_err_die_handler' );
	}
	...
	public function convert_values_from_session() {
		...
		if ( $post && comments_open() ) { // any content that allows comments
			...
			if ( isset( $_SESSION[ $this->session_form_data ] ) && $stored_post_id == $current_post_id ) {
				...
				if ( isset( $_SESSION[ $this->session_form_data ] ) ) {
					unset( $_SESSION[ $this->session_form_data ] );
				}
				if ( isset( $_SESSION[ $this->session_wp_error ] ) ) {
